...
test('printer: comments: beginning of a file', (t) => {
    ...
    transform(ast, {
        plugins: [
            ['insert', {
                ...
                filter: (path) => !path.node.extra.inserted,
                fix: (path) => {
                    path.node.extra.inserted = true;
                    path.node.body.unshift(template.ast(montag`
                        world()
                    `));
                },
            }],
...
test('printer: comment: trailing: line', (t) => {
    t.print(fixture.commentTrailingLine);
    t.end();
});
...
test('printer: comments: trailing: no start', (t) => {
    const source = fixture.commentsTrailingNoStart;
    ...
    t.equal(result, fixture.commentsTrailingNoStartFix);
    t.end();
});
