...
const isLongAssignPattern = (path, semantics) => {
    if (!path.node)
        return false;
...
module.exports.ObjectPattern = {
    print: maybeTypeAnnotation((path, printer, semantics) => {
        ...
        for (const [i, property] of properties.entries()) {
            ...
            const prevAssign = isLongAssignPattern(property.getPrevSibling(), semantics);
            const nextAssign = isLongAssignPattern(property.getNextSibling(), semantics);
            const bothLongAssigns = currentAssign && (nextAssign || prevAssign);
            const oneAssign = currentAssign && !nextAssign && !prevAssign;
            ...
            if ((!oneLongAssign || !oneAssign || bothLongAssigns) && (is || hasObject || prevAssignObject && notInsideFn)) {
                print(',');
                print.newline();
