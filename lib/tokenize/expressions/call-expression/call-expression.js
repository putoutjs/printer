...
import {CallExpressionWithMaxArgs} from './call-expression-with-max-args.js';
...
function isMaxArgs(path, {maxArgsInOneLine}) {
    ...
    return args.length >= maxArgsInOneLine;
}
...
export const CallExpression = maybeParens((path, printer, semantics) => {
    if (isMaxArgs(path, semantics))
        return CallExpressionWithMaxArgs(path, printer);
