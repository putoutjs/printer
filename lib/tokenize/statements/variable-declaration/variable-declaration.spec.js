...
test('printer: tokenizer: variable-declaration: no-declaration', (t) => {
    const source = `
        const a = 5;
    `;
    ...
    ast.program.body[0].declarations = [];
...
test('printer: tokenizer: statement: variable-declaration: const: inside for-of', (t) => {
    t.print(fixture.constInsideForOf);
    t.end();
});

test('printer: tokenizer: statement: variable-declaration: const: inside for', (t) => {
    t.print(fixture.constInsideFor);
    t.end();
});

test('printer: tokenizer: statement: variable-declaration: const: two: before for-of', (t) => {
    t.print(fixture.constTwoBeforeForOf);
    t.end();
});

test('printer: tokenizer: statement: variable-declaration: let: before for', (t) => {
    const source = fixture.letBeforeForOf;
    const ast = parse(source);
    ...
    t.equal(result, fixture.letBeforeForOfFix);
    t.end();
});
