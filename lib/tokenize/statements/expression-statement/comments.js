...
export const printTrailingCommentLine = (path, printer, semantics, {isSameLine, printComment}) => {
    ...
    if (isSameLine)
        print.space();
    else if (!isNext(path) || isNextNotSimple(path))
        print.breakline();
    else
        print.indent();
...
const isNextNotSimple = createTypeChecker([
    ['+: -> !', callWithNext(isSimple)],
]);
...
export const printTrailingCommentBlock = (path, printer, semantics, {isSameLine, printComment}) => {
    const {print, indent} = printer;
